...
$(document).ready(function () {
    $.ajax({
        type: 'POST',
        url: base_url + 'admin/invoices/get_invoice_by_id',
        data: "id=" + invoice_id + "&" + csrfName + "=" + csrfHash,
        dataType: "json",
        success: function (result) {
            csrfName = csrfName;
            csrfHash = result.csrfHash;
            var amount = (+sub_total + +total_tax).toFixed(2);
...
$(document).on('click', '.add-invoice-item', function (e) {
    ...
    if (title != '') {
        ...
        i++;
        var html = '<div class="invoice-item py-1">' +
            '<div class="row">' +
...
$(document).on('click', '.remove-invoice-item', function (e) {
    ...
    var deleted_item_ids = $("#deleted_item_ids").val();
    if (deleted_item_ids == '') {
        deleted_item_ids = id;
    } else {
