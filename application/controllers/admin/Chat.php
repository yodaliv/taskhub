...
class Chat extends CI_Controller
{
    ...
    public function index()
    {
        if (!$this->ion_auth->logged_in()) {
            ...
            $workspace_id = $this->session->userdata('workspace_id');
            if (is_admin()) {
                if (check_module($user_id, 'chat') == 0) {
                ...
                if (check_module($admin_id, 'chat') == 0) {
                ...
                for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
                    ...
                    if ($this->upload->do_upload()) {
                        ...
                        if ($storage_used <= $allowed_storage['storage_allowed'] || $allowed_storage['storage_allowed'] == 0 || empty($allowed_storage['storage_allowed'])) {
                            ...
                            if (file_exists('assets/chats/' . $file_data['file_name'])) {
                                unlink('assets/chats/' . $file_data['file_name']);
                            }
                        }
                    }
                }
    ...
    public function delete_group()
    {
        if (!$this->ion_auth->logged_in()) {
            ...
            if (empty($group_id) || !is_numeric($group_id) || $group_id < 1) {
                redirect($this->session->userdata('role') . '/chat', 'refresh');
                return false;
                exit(0);
            }
