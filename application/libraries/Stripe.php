...
class Stripe
{
    ...
    function __construct()
    {
        ...
        $this->secret_key = (isset($stripe_payment_method['stripe_secret_key'])) ? $stripe_payment_method['stripe_secret_key'] : "";
        $this->publishable_key = (isset($stripe_payment_method['stripe_publishable_key'])) ? $stripe_payment_method['stripe_publishable_key'] : "";
        $this->webhook_secret_key = (isset($stripe_payment_method['stripe_webhook_secret_key'])) ? $stripe_payment_method['stripe_webhook_secret_key'] : "";
        $this->currency_code = (isset($stripe_payment_method['stripe_currency_code'])) ? strtolower($stripe_payment_method['stripe_currency_code']) : "usd";
    ...
    public function create_payment_intent($c_data, $metadata = [], $otherdata = [])
    {

        $c_data['currency'] = $this->currency_code;
        $c_data['description'] = "Software development services";
        $c_data['metadata'] = $metadata;
        $c_data['shipping[name]'] = $otherdata['name'];
        $c_data['shipping[address][line1]'] = $otherdata['line1'];
        $c_data['shipping[address][postal_code]'] = $otherdata['postal_code'];
        $c_data['shipping[address][city]'] = $otherdata['city'];
        $c_data['shipping[address][state]'] = $otherdata['state'];
        $c_data['shipping[address][country]'] = $otherdata['country'];
    ...
    public function create_charge($m_data)
    {
        // $c_data['currency'] = $this->currency_code;
        $url = $this->url . 'v1/payment_intents';
        $method = 'POST';
        $response = $this->curl($url, $method, $m_data);
        $res = json_decode($response['body'], true);
        return $res;
    }
