...
<head>
    ...
    <title>Profile &mdash; <?= get_admin_company_title($this->data['admin_id']); ?></title>
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="row justify-content-center">
                                            <div class="avatar-item col-md-3">
                                                ...
                                                <?php if (isset($user->profile) && !empty($user->profile)) { ?>
                                                    <img alt="image" id="profile_image" src="<?= base_url('assets/backend/profiles/' . $user->profile); ?>" class="img-fluid mb-2">
                                                <?php } else { ?>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="form-group <?= is_client() ? 'col-md-3' : 'col-md-4' ?> col-12">
                                                <label><?= !empty($this->lang->line('label_contact')) ? $this->lang->line('label_contact') : 'Phone'; ?> <?php if (is_admin()) { ?><small>(Will be used as company contact)</small><?php }?></label>
                                                <input name="phone" class="form-control" value="<?= $user->phone ?>">
                                            </div>
                                            <?php if (is_client()) { ?>
                                                <div class="form-group col-md-3 col-12">
                                                    <label><?= !empty($this->lang->line('label_company')) ? $this->lang->line('label_company') : 'Company'; ?> <?php if (is_admin()) { ?> <small>(Will be used as company name)</small><?php }?></label>
                                                    <input name="company" class="form-control" value="<?= $user->company ?>">
                                                </div>
                                            <?php } ?>
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label for="address"><?= !empty($this->lang->line('label_address')) ? $this->lang->line('label_address') : 'Address'; ?> <?php if (is_admin()) { ?><small>(Will be used as company address)</small><?php }?></label>
                                                    <textarea type="textarea" class="form-control" placeholder="Address" name="address" id="address"><?= $user->address ?></textarea>
                                                </div>
                                            </div>

                                            <div class="form-group col-md-6 col-12">
                                                <label><?= !empty($this->lang->line('label_city')) ? $this->lang->line('label_city') : 'City'; ?> <?php if (is_admin()) { ?><small>(Will be used as company city)</small><?php } ?></label>
                                                <input name="city" class="form-control" placeholder="City" value="<?= $user->city ?>">
                                            </div>

                                            <div class="form-group col-md-4 col-12">
                                                <label><?= !empty($this->lang->line('label_state')) ? $this->lang->line('label_state') : 'State'; ?> <?php if (is_admin()) { ?><small>(Will be used as company state)</small><?php }?></label>
                                                <input name="state" class="form-control" placeholder="State" value="<?= $user->state ?>">
                                            </div>

                                            <div class="form-group col-md-4 col-12">
                                                <label><?= !empty($this->lang->line('label_zip_code')) ? $this->lang->line('label_zip_code') : 'Zip Code'; ?> <?php if (is_admin()) { ?><small>(Will be used as company zipcode)</small><?php }?></label>
                                                <input name="zip_code" class="form-control" placeholder="Zip Code" value="<?= $user->zip_code ?>">
                                            </div>

                                            <div class="form-group col-md-4 col-12">
                                                <label><?= !empty($this->lang->line('label_country')) ? $this->lang->line('label_country') : 'Country'; ?> <?php if (is_admin()) { ?><small>(Will be used as company country)</small><?php }?></label>
                                    ...
                                    <div class="card-footer text-right">
                                        <button class="btn btn-primary" id="updt_prfl_btn"><?= !empty($this->lang->line('label_save_changes')) ? $this->lang->line('label_save_changes') : 'Save Changes'; ?></button>
                                    </div>
                                    </form>
                                </div>
                            </div>
